...
class Simple_calendar {
  ...
  create() {
    ...
    this.update();
    //时间刷新
    self.setInterval('Simple_calendar.timeupdate()', 200);
  }
  ...
  addData(year, month) {
    ...
    for (var i = 0; i < 42; i++) {
      ...
      if (this._options.showLunarCalendar) {
        var a =  new LunarHelp(writeyear, writemonth, writeday).getLunarDayName();
...
//时间刷新函数
Simple_calendar.timeupdate = function () {
...
//国际化，和一些节日数据，标记数据
Simple_calendar.prototype.languageData = {
  feativals_CH: {
    '1-1': '元旦',
    '2-14': '情人节',
    '3-8': '妇女节',
...
Simple_calendar.prototype.tyear = new Date().getFullYear();
Simple_calendar.prototype.tmonth = new Date().getMonth() + 1;
Simple_calendar.prototype.tday = new Date().getDate();
