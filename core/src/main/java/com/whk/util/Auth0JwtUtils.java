...
public class Auth0JwtUtils {
    ...
    public static String sign(Map<String, Object> map) {
        try {// 设置过期时间
            ...
            header.put("typ", "jwt");
            // 返回token字符串
            JWTCreator.Builder builder = JWT.create().withHeader(header).withIssuedAt(new Date())
                    //发证时间
                    .withExpiresAt(date);
            //过期时间
            // .sign(algorithm);
...
//            map.entrySet().forEach(entry -> builder.withClaim(entry.getKey(), (Boolean) entry.getValue()));
    ...
    /**
     * 检验token是否正确 * @param **token** * @return
     */
    ...
    /**
     * 验证token是否失效 * * @param token * @return true:过期 false:没过期
     */
    ...
    /**
     * 直接Base64解密获取header内容 * @param token * @return
     */
    ...
    /**
     * 直接Base64解密获取payload内容 * @param token * @return
     */
